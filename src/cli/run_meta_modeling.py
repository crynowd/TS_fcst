...
from src.config.loader import load_meta_modeling_config
from src.meta_modeling import run_meta_modeling_pipeline
from src.utils.logging_utils import setup_logger


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run regression-based meta-modeling pipeline")
    parser.add_argument(
        "--config",
        default="configs/meta_modeling_v1.yaml",
        help="Path to meta-modeling config",
    )
...
def main() -> None:
    args = parse_args()
    cfg = load_meta_modeling_config(args.config)
    ...
    logger.info("Meta-modeling config loaded: %s", cfg["meta"]["config_path"])
