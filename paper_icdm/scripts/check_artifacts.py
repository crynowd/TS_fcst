...
FIGURE_BUILDER = "paper_icdm/scripts/build_paper_figures.py"
GENERATED_FIGURE_ARTIFACTS = [
...
def check_paper_coverage(reporter: Reporter) -> None:
    ...
    rows = [
        ...
        ("Figure 2", "Table VI display-scale CSV", repo_path("paper_icdm/tables/table_vi_meta_selection_results.csv").exists(), "OK"),
        ("Figure 3", "Table VI display-scale CSV", repo_path("paper_icdm/tables/table_vi_meta_selection_results.csv").exists(), "OK"),
    ]
    print("\nCoverage matrix:")
...
def check_paper_figures(reporter: Reporter) -> None:
    section = "Paper figures"
    builder_path = repo_path(FIGURE_BUILDER)
    if builder_path.exists() and builder_path.is_file():
        reporter.ok(section, f"{FIGURE_BUILDER} present ({file_size(builder_path)})")
    else:
        reporter.fail(section, f"{FIGURE_BUILDER} missing")

    for relative_path in GENERATED_FIGURE_ARTIFACTS:
        path = repo_path(relative_path)
        if path.exists() and path.is_file():
            reporter.ok(section, f"{relative_path} present ({file_size(path)})")
        elif builder_path.exists():
            reporter.warning(section, f"{relative_path} missing; run {FIGURE_BUILDER} to regenerate")
        else:
            reporter.warning(section, f"{relative_path} missing; figure builder is not available")

    for relative_path in FIGURE_SUPPORT_TABLES:
        path = repo_path(relative_path)
        if path.exists() and path.is_file():
            reporter.ok(section, f"{relative_path} present ({file_size(path)})")
        elif builder_path.exists():
            reporter.warning(section, f"{relative_path} missing; generated by {FIGURE_BUILDER}")
        else:
            reporter.warning(section, f"{relative_path} missing; figure builder is not available")

    reporter.warning(section, "Figure 1 remains a manual/schematic figure and is not generated by build_paper_figures.py")
