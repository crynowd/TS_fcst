...
Build paper figures 2-4 from the generated compact tables:

```bash
python paper_icdm/scripts/build_paper_figures.py
```

The figure builder reads `paper_icdm/tables/table_vi_meta_selection_results.csv` for Figures 2 and 3, using the paper display-scale RMSE and directional-accuracy columns. It reads `paper_icdm/tables/table_v_winner_family_counts_by_horizon.csv` for Figure 4; if that support table is absent, it derives it from `artifacts/forecasting/forecasting_benchmark_v2/metrics_long.parquet` and `paper_icdm/model_family_mapping.csv` with the same fold-mean series-level winner logic used by Table V. Figure 1 remains manual/schematic and is not data-generated by this script.
...
- Figure 2 and 3 can be built from Table VI display-scale CSV values;
- Figure 4 can be built from the by-horizon winner-count support table;
...
- Figure builder output should be regenerated after any source table changes with `python paper_icdm/scripts/build_paper_figures.py`.
